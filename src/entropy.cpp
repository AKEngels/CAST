...
namespace entrop
{
      ...
      else
      {
        for (size_t i = Config::get().entropy.entropy_start_frame_num; j < coordsMatrix.rows(); ++j, i += Config::get().entropy.entropy_offset)
        {
          ...
          if (Config::get().entropy.entropy_alignment)
          {
            ...
            ::matop::align::centerOfMassAlignment(coords); 
            // Rotational alignment
            ::matop::align::kabschAlignment(coords, coords_ref); 
          }
  ...
  float_type TrajectoryMatrixRepresentation::hnizdo(size_t const k)
  {
    ...
    // Otherwise we may work with a reference
      ...
      float_type distance = std::log(std::sqrt(::matop::entropy::knn_distance(input, input.rows(), k, (size_t)0u, (size_t)0u)));
#ifdef _OPENMP
...
#pragma omp for
        ...
        for (int k = 1; k < (int)input.cols(); k++)
        {
          ...
          distance += std::log(std::sqrt(::matop::entropy::knn_distance(input, input.rows(), k, (size_t)0u, (size_t)k, buffer)));
        }
        delete[] buffer;
#ifdef _OPENMP
      }
...
#pragma omp parallel for firstprivate(input, k) shared(marginal_entropy_storage)
    ...
    for (int i = 0; i < (int)input.rows(); i++)
    {
      ...
      {
        distance += log(sqrt(::matop::entropy::knn_distance(input, 1, k, (size_t)i, k, buffer))); // set eucledean distance to ouptut
      }
...
#pragma omp parallel for firstprivate(pca_modes, k) shared(entropy_anharmonic, entropy_mi)
    ...
    for (int i = 0; i < (int)pca_modes.rows(); i++)
    {
      ...
      float_type distance = 0.0;
      {
        ...
        {
          distance += log(sqrt(::matop::entropy::knn_distance(pca_modes, 1, k, i, k, buffer)));
        }
      }
      ...
      for (size_t j = i + 1; j < pca_modes.rows(); j++)
      {
        ...
        for (size_t k = 0; k < pca_modes.cols(); k++)
        {
          distance += log(sqrt(::matop::entropy::knn_distance(pca_modes, 2, k, query_rows, k, buffer)));
        }
