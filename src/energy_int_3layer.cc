...
energy::interfaces::three_layer::THREE_LAYER::THREE_LAYER(coords::Coordinates *cp):
  ...
  qm_energy(0.0), se_energy_small(0.0), se_energy_middle(0.0), mm_energy_middle(0.0), mm_energy_big(0.0),
	index_of_middle_center(qmmm_helpers::get_index_of_QM_center(Config::get().energy.qmmm.center, qm_se_indices, coords)),
...
coords::float_type energy::interfaces::three_layer::THREE_LAYER::qmmm_calc(bool if_gradient)
{
  ...
  if (Config::get().energy.qmmm.zerocharge_bonds != 0)
  {
    ...
    qmmm_helpers::add_external_charges(qm_se_indices, qm_se_indices, mmc_big_charges, all_indices, link_atoms_middle, charge_indices, coords, index_of_middle_center);
  }
  ...
  if (if_gradient && integrity == true && Config::get().energy.qmmm.zerocharge_bonds != 0)
  {
    ...
    for (auto i=0u; i<charge_indices.size(); ++i)
    {
      ...
      auto grad_mmc_small = mmc_middle_g_ext_charges[i];
			...
			if (Config::get().energy.qmmm.cutoff != std::numeric_limits<double>::max())
			{
				...
				for (auto j{ 0u }; j < qm_se_indices.size(); ++j)
				{
					...
					sum_of_QM_interactions_sec += (QMcharge_sec * MMcharge * elec_factor) / dist;
					sum_of_QM_interactions_mmc += (QMcharge_mmc * MMcharge * elec_factor) / dist;
				}

				// additional gradient on external charge due to interaction with SEC
				...
				// additional gradient on external charge due to interaction with MMC
				...
				new_grads[Config::get().energy.qmmm.center] += (derivQ_mmc - derivQ_sec);
			}
    ...
    else if (Config::get().energy.qmmm.emb_small > 1)  // EE+ and MMSE
    {
      ...
      charge_indices.clear();
        ...
        if (sec_middle.size() == 0) throw std::runtime_error("no charges found in SE interface");
				qmmm_helpers::add_external_charges(qm_indices, qm_indices, sec_middle_charges, qm_se_indices, link_atoms_small, charge_indices, coords, index_of_small_center);   // add charges from SE atoms
			}
      
      auto mmc_big_charges = mmc_big.energyinterface()->charges();
      if (mmc_big_charges.size() == 0) throw std::runtime_error("no charges found in MM interface");
      auto all_indices = range(coords->size());
      qmmm_helpers::add_external_charges(qm_indices, qm_se_indices, mmc_big_charges, all_indices, link_atoms_small, charge_indices, coords, index_of_small_center);     // add charges from MM atoms
    }
  }
  ...
  Config::set().periodics.periodic = false;
	...
	if (Config::get().energy.qmmm.emb_small != 0 && if_gradient && integrity == true && Config::get().energy.qmmm.zerocharge_bonds != 0)
	{
		...
		for (auto i = 0u; i<charge_indices.size(); ++i)
		{
			...
			auto grad_sec_small = sec_small_g_ext_charges[i];
			...
			if (Config::get().energy.qmmm.cutoff != std::numeric_limits<double>::max())
			{
				...
				grad_sec_small += derivQ_sec;

				// additional gradient on QM atom that defines distance
				new_grads[Config::get().energy.qmmm.center] += (derivQ_sec - derivQ_qmc);
			}

			new_grads[mma] += grad_qmc;
			new_grads[mma] -= grad_sec_small;
		}
	}
