...
TEST(qmmm, test_number_of_link_atoms)
{
  ...
  auto linkatoms = energy::interfaces::qmmm::create_link_atoms(qm_indizes, &coords, tp, { 85 });
...
TEST(qmmm, test_gradient_of_link_atom)
{
  ...
  auto linkatom = energy::interfaces::qmmm::create_link_atoms(qm_indizes, &coords, tp, { 85 })[0];
  ...
  energy::interfaces::qmmm::calc_link_atom_grad(linkatom, g_la, &coords, g_qm, g_mm);
...
TEST(qmmm, test_get_mm_atoms)
{
  ...
  auto mm_atoms = energy::interfaces::qmmm::get_mm_atoms(15);
...
{
  std::vector<size_t> qm_indizes = { 5,8,9,10,11,12,13,14 };
  auto new_indizes = energy::interfaces::qmmm::make_new_indices(qm_indizes, 15);
...
TEST(qmmm, test_make_new_indices_for_mm_atoms)
{
  ...
  auto new_indizes = energy::interfaces::qmmm::make_new_indices(mm_indizes, 15);
...
TEST(qmmm, test_small_coords_for_mm_system)
{
  ...
  auto small_coords = energy::interfaces::qmmm::make_small_coords(&coords, mm_indizes, new_indizes, config::interface_types::OPLSAA, "MM System: ");
...
TEST(qmmm, test_small_coords_for_qm_system)
{
  ...
  auto linkatoms = energy::interfaces::qmmm::create_link_atoms(qm_indizes, &coords, tp, { 85 });
  auto small_coords = energy::interfaces::qmmm::make_small_coords(&coords, qm_indizes, new_indizes, config::interface_types::DFTB, "QM system: ", false, linkatoms);
...
TEST(qmmm, test_get_index_of_qmcenter)
{
  ...
  auto index_of_QMcenter = energy::interfaces::qmmm::get_index_of_QM_center(0, qm_indizes, &coords);
...
TEST(qmmm, test_add_external_charges_M1)
{
  ...
  energy::interfaces::qmmm::add_external_charges(qm_indizes, charges, all_indizes, linkatoms, result, &coords, 8);
...
TEST(qmmm, test_add_external_charges_onlySE)
{
  ...
  std::vector<int> result;
  energy::interfaces::qmmm::add_external_charges(qm_indizes, charges, qmse_indizes, linkatoms, result, &coords, 8);
