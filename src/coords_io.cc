...
coords::input::format* coords::input::new_format(void) {
  ...
  case config::input_types::PDB:
    ...
    break;
  default: { return new formats::tinker; }
  }
}
 ...
 */
coords::Coordinates
coords::input::formats::tinker::read(std::string const& file) {
...
std::ostream& coords::operator<<(std::ostream& stream,
                                 coords::Coordinates const& coord) {
...
static void tinker_dummy_to_stream(std::ostream& stream, std::size_t index,
                                   coords::float_type x, coords::float_type y,
                                   coords::float_type z) {
  stream << std::right << std::setw(6) << index << "  ";
  stream << std::left << "XX ";
  stream << std::fixed << std::showpoint << std::right << std::setw(12)
...
void coords::output::formats::tinker::to_stream(std::ostream& stream) const {
  bool const pp = Config::get().periodics.periodic &&
                  Config::get().periodics.periodic_print;
  ...
  for (std::size_t i(0U); i < N; ++i) {
    ...
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).x();
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).y();
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).z();
    ...
    if (ref.atoms(i).sub_type() == coords::Atom::sub_types::ST_IN)
      stream << " IN";
    else if (ref.atoms(i).sub_type() == coords::Atom::sub_types::ST_OUT)
      stream << " OUT";
    stream << '\n';
  }
  if (pp) {
    coords::Cartesian_Point const halfbox = Config::get().periodics.pb_box / 2.;
    tinker_dummy_to_stream(stream, N + 1, halfbox.x(), halfbox.y(),
                           -halfbox.z());
    tinker_dummy_to_stream(stream, N + 2, -halfbox.x(), -halfbox.y(),
                           -halfbox.z());
    tinker_dummy_to_stream(stream, N + 3, halfbox.x(), -halfbox.y(),
                           -halfbox.z());
    tinker_dummy_to_stream(stream, N + 4, -halfbox.x(), halfbox.y(),
                           -halfbox.z());
    tinker_dummy_to_stream(stream, N + 5, halfbox.x(), halfbox.y(),
                           halfbox.z());
    tinker_dummy_to_stream(stream, N + 6, -halfbox.x(), -halfbox.y(),
                           halfbox.z());
    tinker_dummy_to_stream(stream, N + 7, halfbox.x(), -halfbox.y(),
                           halfbox.z());
    tinker_dummy_to_stream(stream, N + 8, -halfbox.x(), halfbox.y(),
                           halfbox.z());
  }
}
...
void coords::output::formats::moldenxyz::to_stream(std::ostream& stream) const {
  ...
  for (std::size_t i(0U); i < N; ++i) {
    stream << std::left << std::setw(3)
           << atomic::symbolMap[ref.atoms(i).number()];
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).x();
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).y();
...
void coords::output::formats::xyz_mopac7::to_stream(
...

  {
    if (ref.atoms(i).fixed()) {
      stream << std::left << std::setw(3)
             << atomic::symbolMap[ref.atoms(i).number()];
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).x() << " 0";
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).y() << " 0";
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).z() << " 0";
      stream << '\n';
    } else {
      stream << std::left << std::setw(3)
             << atomic::symbolMap[ref.atoms(i).number()];
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).x() << " 1";
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).y() << " 1";
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
             << std::setprecision(6) << ref.xyz(i).z() << " 1";
      stream << '\n';
    }
  }
}
...
/**writes the geometry of the coord object in the gen format (description of
 * format see manual of dftb+, appendix C)*/
void coords::output::formats::xyz_gen::to_stream(std::ostream& stream) const {
  ...
  stream << "\n";
  for (std::size_t i(0U); i < N; ++i) {
    stream << std::left << std::setw(5) << i + 1 << std::left << std::setw(5)
           << find_index(ref.atoms(i).symbol(), existing_symbols) + 1;
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).x();
    stream << std::fixed << std::showpoint << std::right << std::setw(12)
           << std::setprecision(6) << ref.xyz(i).y();
...
void coords::output::formats::zmatrix::to_stream(std::ostream& stream) const {
  ...
  if (stream.good() && N > 0) {
    ...
    stream << std::left << std::setw(4)
           << ref.atoms(ref.atoms(0u).i_to_a()).symbol() << '\n';
    if (N > 1) {
    ...
    if (N > 1) {
      ...
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(1u).radius();
      stream << '\n';
    }
    if (N > 2) {
      stream << "bnd" << std::left << std::setw(10) << 2u;
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(2u).radius();
      stream << '\n';
      stream << "ang" << std::left << std::setw(10) << 2u;
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(2u).inclination();
      stream << '\n';
    }
    for (std::size_t i = 3; i < N; ++i) {
      stream << "bnd" << std::left << std::setw(10) << i;
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(i).radius();
      stream << '\n';
      stream << "ang" << std::left << std::setw(10) << i;
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(i).inclination();
      stream << '\n';
      stream << "dih" << std::left << std::setw(10) << i;
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.intern(i).azimuth();
      stream << '\n';
    }
    stream << "constants\n";
    for (std::size_t i = 1; i < N; ++i) {
      ...
      stream << std::right << std::fixed << std::setw(20)
             << std::setprecision(6) << ref.g_intern(i).x();
      stream << '\n';
      if (i > 1) {
        stream << "g_ang" << std::left << std::setw(10) << i;
        stream << std::right << std::fixed << std::setw(20)
               << std::setprecision(6) << ref.g_intern(i).y();
        stream << '\n';
      }
      if (i > 2) {
        stream << "g_dih" << std::left << std::setw(10) << i;
        stream << std::right << std::fixed << std::setw(20)
               << std::setprecision(6) << ref.g_intern(i).z();
        stream << '\n';
      }
    }
    stream << "end\n";
  } else
    throw std::runtime_error("ERR_FILE_WRITE: stream bad");
}
...
void coords::output::formats::xyz_mopac::to_stream(std::ostream& stream) const {
  ...
  for (std::size_t i(0U); i < N; ++i) {
    /* stream << std::left  << std::setw(3) <<
    atomic::symbolMap[ref.atoms(i).number()]; stream << std::fixed <<
    std::showpoint << std::right << std::setw(12) << std::setprecision(6) <<
    ref.xyz(i).x(); stream << std::fixed << std::showpoint << std::right <<
    std::setw(12) << std::setprecision(6) << ref.xyz(i).y(); stream <<
    std::fixed << std::showpoint << std::right << std::setw(12) <<
    std::setprecision(6) << ref.xyz(i).z(); stream << '\n';*/
    if (ref.atoms(i).fixed()) {
      stream << std::left << std::setw(3)
             << atomic::symbolMap[ref.atoms(i).number()];
      stream << std::fixed << std::showpoint << std::right << std::setw(12)
