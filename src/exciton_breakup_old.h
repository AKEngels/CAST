...
int exciton_breakup(int pscanzahl, int numberOf_n_SC, char ebene, std::string masscenters, std::string nscpairrates,
  std::string pscpairexrates, std::string pscpairchrates, std::string pnscpairrates)
{
  ...
  if (gesamtanzahl == pscanzahl + numberOf_n_SC) //test if correct number of molecules was given
  {
    std::cout << "Number of monomers is correct, proceeding." << std::endl;
  }
  else //gesamtanzahl != pscanzahl + numberOf_n_SC
  {
    std::cout << "Wrong number of monomers detected!" << std::endl;
    throw std::logic_error("Wrong numbers of p- and n-type molecules in inputfile. Expected: " + std::to_string(gesamtanzahl) + " | Is: " + std::to_string(pscanzahl + numberOf_n_SC));
    return -1;
  }
  ...
  exciton.close();
  std::vector <int> partneranzahl(pscanzahl + numberOf_n_SC + 1);
  for (std::size_t i = 1u; i < (pscanzahl + numberOf_n_SC + 1); i++)
  {
    partneranzahl[i] = 0;
