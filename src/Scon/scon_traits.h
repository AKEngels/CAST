...
namespace scon {
  ...
  namespace is_container_impl {
    ...
    template <typename... Args>
    struct is_container<std::map<Args...>> : std::true_type {};
    template <typename... Args>
    struct is_container<std::unordered_set<Args...>> : std::true_type {};
    template <typename... Args>
    struct is_container<std::unordered_map<Args...>> : std::true_type {};
    template <typename... Args>
    struct is_container<scon::vector<Args...>> : std::true_type {};
  } // namespace is_container_impl

  template <typename T>
  struct is_container : is_container_impl::is_container<std::decay_t<T>> {};

  template <typename PtrDeriv, typename PtrBase>
  std::unique_ptr<PtrDeriv>
    dynamic_unique_cast(std::unique_ptr<PtrBase> to_cast) {
  ...
  template <bool B>
  struct Boolean : std::true_type {};
  template <>
  struct Boolean<false> : std::false_type {};
  ...
  struct And<A, Args...> : Boolean<A&& And<Args...>::value> {};

  template <bool A>
  struct And<A> : Boolean<A> {};

  // Not

  template <class T>
  using Not = Boolean<!T::value>;
  ...
  template <class T>
  using is_range = trait_detail::is_range<T>;

  template <class T>
  using range_value = typename trait_detail::range_value_type<T>::type;

  template <class T>
  using range_begin_iterator =
    typename trait_detail::range_begin_iterator_type<T>::type;

  template <class T>
  using range_end_iterator =
  ...
  template <>
  struct float_helper<float> {
    using type = float;
  };
  template <>
  struct float_helper<long double> {
  ...
  struct is_convertible_from : std::is_constructible<T, Args...> {};

  template <typename T, typename U>
  struct is_convertible_from<T, U> : std::is_convertible<U, T> {};
...
#define SCON_PERFECT_FORWARDING_WRAPPER_CONSTRUCTORS(classname, wrapped_type,  \
                                                     wrapped_name)             \
  template <typename... Args,                                                  \
            scon::EnableIf<scon::is_convertible_from<wrapped_type, Args...>> = \
                0>                                                             \
  ...
  explicit classname(T&& arg) : wrapped_name(std::forward<T>(arg)) {}          \
                                                                               \
  template <class T, scon::EnableIf<std::is_convertible<                       \
                         std::initializer_list<T>, wrapped_type>> = 0>         \
  classname(std::initializer_list<T> list) : wrapped_name(list) {}             \
                                                                               \
  template <class T, class... Args,                                            \
            scon::EnableIf<std::is_constructible<                              \
                wrapped_type, std::initializer_list<T>, Args...>> = 0>         \
  classname(std::initializer_list<T> list, Args&&... args)                     \
      : wrapped_name(list, std::forward<Args>(args)...) {}
