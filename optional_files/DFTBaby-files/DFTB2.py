...
"erfgau" => erf(R_AB/R_lr) - 2/(sqrt(pi)*R_lr)*R_AB*Exp(-1/3 (R_AB/R_lr)^2)
        ...
        Parametrization.fluctuation_functions: Functional form of the spherical charge fluctuations dn(r) on which the gamma-matrix is based, can be 'Gaussian' or 'Slater'
        ...
        if parameter_set == "homegrown":
            ...
            reppot_tables.load_atompairs(atompairs)
        self.SKT = {}
        self.VREP = {}
        for (Zi,Zj) in atompairs:
            ...
            try:
                if parameter_set == "homegrown":
                    ...
                    # load repulsive potential, for HCNO atoms I just copied the repulsive potential of hotbit
                    reppot_module = reppot_tables.atompairs[(Zi,Zj)]
                    # If I combine my own electronic parametrization with Hotbit's 
                    # repulsive potentials the bond lengths are a little bit too large
                    # To make the core potentials less repulsive, the distance-axis
                    # is scaled by a factor of 0.95-1.00 for each atom pair.
                    scaling_factors = hotbit_scaling_factors
                    ...
                    if parameter_set == "mio":
                        ...
                        slako_module = read_slakoformat("%s/dftb-plus_parameters/mio-0-1/%s-%s.skf" % (dftb_dir,atom_names[Zi-1].capitalize(), atom_names[Zj-1].capitalize()))
                        reppot_module = dummy_reppot_module()
                    elif parameter_set == "hotbit":
        ...
        if self.fluctuation_functions == "Gaussian":
            ...
            raise ValueError("fluctuation_functions can be 'Gaussian' or 'Slater' but not %s" % self.fluctuation_functions)
...
def main(xyz_file, conf_file):
    ...
    usage += "   in a configuration file called '{}'\n".format(conf_file)
...
if __name__ == "__main__":
    print main(sys.argv[1], "dftbaby.cfg")
