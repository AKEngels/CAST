...
hotbit_scaling_factors = defaultdict(
    lambda: 0.96, # default factor for atom pairs not specified below
  { # make hotbit core potentials less repulsive
    (1,1) : 0.95, # H-H
    (1,6) : 0.80, # H-C
    (1,7) : 0.95, # H-N
    (1,8) : 0.93, # H-O
    ...
    (6,7) : 0.96, # C-N
    (6,8) : 0.96,  # C-O
    # repulsive potentials for silicon are copied from carbon
    # silicon has more core electrons, so the potential should
    # be more repulsive than for carbon
    (14,14): 1.2, # Si-Si
  })

class RepulsivePotential:
    def __init__(self, reppot_module, scaling_factors=defaultdict(lambda: 1.0,{}), smooth_decay=True):
        """
        Parameters:
        ===========
        ...
        if smooth_decay:
            ...
            dmax_indx = argmax(reppot_module.d[reppot_module.Vrep >= 0.1])
            ...
            def smooth_tail(d,deriv=0):
                return 0.0
        reppot_module.d /= scaling   # necessary if programme is called more than once (in CAST)
