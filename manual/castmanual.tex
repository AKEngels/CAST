...
A link atom approach is implemented to run a bonded QM/MM calculation. The atom type of the link atoms must be given by the option ``QMMMlinkatomtype''. This option must be given as often as the number of link atoms in your system and the types must be in the order of the QM atoms that correspond to the link atoms. Link atoms can only be hydrogen atoms. How to deal with charges can be determined by the option ``QMMMzerocharge\_bonds''. If it's set to 1, the M1 charges (i.\,e. charges of atoms that are connected to the QM system by 1 bond) are switched off for coulomb interactions between QM and MM system. If it's set to 2 M2 charges (i.\,e. charges of atoms that are connected to the QM system by 2 bonds) are also set to zero. All the other MM charges are used ``normally'', i.\,e. they are not scaled down. Furthermore there is a cutoff (given by ``QMMMcutoff''). MM atoms whose distance to the nearest QM atom is more than this are not taken into account for the electrostatic interactions.
