...
\end{align}
	...
	\begin{tabularx}{\textwidth}{l|X|X}
		...
		\textbf{FEPvdwcouple} & Controls coupling of \ac{VdW} interactions; with \mbox{x = FEPvdwcouple}; if \mbox{x = 1}: $\lambda_{vdw,out} = 1 - \lambda$ and $\lambda_{vdw,in} = \lambda$, else $\lambda_{vdw,out} = 1 - \frac{|\lambda-x|}{1-x}$ and $\lambda_{vdw,in} = \frac{|\lambda-x|}{1-x}$; only makes sense if $x < 0.5$ \fi & float [1.0] \\
