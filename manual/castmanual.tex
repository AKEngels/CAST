...
There is also the possibility to use QM/MM as energy interface. The QM part is calculated with MOPAC, for the MM part you can choose one of the force fields OPLSAA, CHARMM and AMBER. Interactions between QM and MM part are only calculated as non-bonded interactions so there must not be any bonds between these systems. As the van der Waals part of the interaction is done by force field potential the whole system must be parametrized with the force field you use for the MM part.

To use this interface set the energy interface to ``QMMM'' (keyword ``interface''). You furthermore have to give all options for your forcefield (especially the parameter file ``paramfile'') and for MOPAC. The keywords for MOPAC must contain ``QMMM AUX'' because you want to do a QM/MM calculation and the information you need is read from an auxiliary file (for details about a MOPAC QM/MM calculation see http://openmopac.net/manual/QMMM.html). 

By ``QMMMqatoms'' you define which atoms belong to the QM system (of course all other atoms then belong to the MM system). Write the indizes in TINKER indexation (i.\,e. starting with 1) either separated by comma (``1,5,7'') or as a range (``1-4''). The keyword ``QMMMmminterface'' tells CAST which forcefield to use as MM interface (this has to be in accordance with the parameter file and the parametrization of your input structure.
...
\end{lstlisting}	
	...
	\subsection{CUT\_RESIDUES}
	
	This task takes a structure from a PDB input file (see section \ref{sec:pdb}) and cuts out a smaller structure around a user determined reaction site. You can determine the active site by the keyword ``CUTreact\_atom''. Use this keyword several times to add the atoms numbers of the atoms that make up the reaction site (numbering starting with 1). By the keyword ``CUTdist'' you determine the size of your new structure (distance from active site that is the geometrical center of all reactive atoms). However this task just cuts between residues so if one atom of a residue is inside that radius the whole residue is kept. If a peptide is cut the N-terminus is complemented by a NH\textsubscript{3}\textsuperscript{+} group and the C-terminus is complemented by a COO\textsuperscript{-} group. Because of the stuff with the residues this task only works with PDB as input. The new (smaller) structure is written into a new .arc file in tinker format. The atom types that are assigned are similar to those of an OPLSAA force field but can't be used for force field calculations. If you want to perform a follow-up calculation with the output structure where you fix the protein backbone, copy the content of the file ``fix.txt'' to your inputfile ``CAST.txt''. 
