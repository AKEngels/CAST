...
\end{itemize}
	...
	\fi
		%% MOPAC
		\subsubsection{MOPAC}
		\ac{MOPAC} is accessed via system call. The \ac{MOPAC} interface expects the \ac{MOPAC} executable path to be either \glqq \textbackslash opt\textbackslash mopac\textbackslash MOPAC2016.exe\grqq on Linux \ifverbose (yes, the executable default fileending is .exe even on Linux-Systems) \fi or \glqq C:\textbackslash Program Files\textbackslash mopac\textbackslash MOPAC2016.exe\grqq~on Windows systems by default. The path can be changed using the config-variable \glqq MOPACpath\grqq.~\\
...
A link atom approach is implemented to run a bonded QM/MM calculation. The atom type of the link atoms must be given by the option ``QMMMlinkatomtype''. This option must be given as often as the number of link atoms in your system and the types must be in the order of the QM atoms that correspond to the link atoms. Link atoms can only be hydrogen atoms. How to deal with charges can be determined by the option ``QMMMzerocharge\_bonds''. The number you give here is the number of bonds from the inner region up to which the charges of atoms are set to zero when creating external charges for the QM calculation. So if it's 1 only charges of atoms that are directly bonded to the QM system are deleted, if it's set to 2 one more layer of atoms is not taken into account and so on. Currently you can choose a number between 1 and 3. Furthermore there is a cutoff (given by ``QMMMcutoff''). MM atoms whose distance to the nearest QM atom is more than this are not taken into account for the electrostatic interactions.
...
\end{itemize}
    ...
    \textbf{\underline{Startopt Options}} &  &  \\ 
		\textbf{SOtype} & what kind of calculation will be performed? \begin{itemize} \item 0 = Ringsearch \item 1 = Solvadd \item 2 = Ringsearch + Solvadd \end{itemize}	& int [1]\\
		\textbf{SOstructures} & ??? & ??? \\
		\hline
