...
For running DFTB calculations an interface to DFTBaby (\url{http://userpage.fu-berlin.de/humeniuka/DFTBaby/mdwiki.html#!WIKI/main_page.md}) is implemented. Because DFTBaby is written in python CAST needs to be set up to deal with python. This can be done by premake. Just use premake as usual and choose the configuration Python\_Release or Python\_Debug. To be able to do this you have to install python before. \textbf{IMPORTANT}: If you want to compile CAST in the 32bit modus you have to install python 32bit, if you want to compile CAST in 64bit modus you have to install python 64bit!

\textbf{For users on ECPC}: The python 2.7 version that is installed on ECPC is 64bit so you can only compile CAST with python in 64bit mode. Also you have to add the following line to your .cshrc so CAST can find python:
\begin{lstlisting}
...
If you want to run CAST with DFTB, set the interface parameter in the CAST inputfile to DFTB. To interact with DFTBaby it is necessary to copy the file ``dftbaby\_interface.py'' into the folder where you run CAST. You also have to provide the path to DFTBaby (.../DFTBaby-\#\#\#) as DFTBath. Now you can run your calculation. The output of DFTBaby is written into the file ``output\_dftb.txt''. All DFTB options apart from DFTBath are optional. 
...
\end{align}
	...
	\ac{FEP} calculations produce three output files: ``alchemical.txt'', ``FEP\_Results.txt'' and ``dE\_pot.txt''. ``alchemical.txt'' contains detailed information about electrostatic and \ac{VdW} interactions for the current lambda value. Furthermore, the temperature and free energy change is displayed. The columns are:
	\begin{itemize}
	...
	The file ``FEP\_Results.txt'' contains the total free energy change for the simulation and for each window. The first column is the $\lambda$-Value where the current window ends, the second line the total free energy change until this window from the forward transformation, the third line the total free energy change until this window from the backward transformation. The total free energy changes for forward and backward simulation should be similar (in theory identical).
	
	The third output file ``dE\_pot.txt'' contains the $\Delta E_{pot}$ values for all conformations in all windows for forward and backwards transformation. This file can be analysed by the script ``FEP\_histograms.py''. Just copy the script in the same folder as the output file and run it. It produces a picture for every window where the probability distributions of the potential energy differences of forward and backwards transformation for every window are plotted. Furthermore the overlap of these distributions for every window is calculated and printed. The quality of a calculation is better if the overlap between forward and backwards distribution is bigger.
