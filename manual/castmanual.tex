...
\textbf{Attention!} If the number of conformations for each production run is too large there can be instabilities in the calculation of the dG value via the sample average. You can see this in the outputfile ``alchemical.txt'' where the dG value changes very rapidly during the production run. Another possibility to check if the results are reasonable is the use of the script ``FEP\_check.py''. The results given there should be in the same magnitude as those in the outputfile ``FEP.txt''
