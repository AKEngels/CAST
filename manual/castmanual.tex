...
\begin{lstlisting}[firstnumber=92]
				    ...
				    self.dmax = max(reppot\_module.d)
						self.tck = interpolate.splrep(reppot\_module.d, reppot_module.Vrep,s=0)
						def smooth_tail(d,deriv=0)
						    return 0.0
				reppot\_module.d /= scaling
				self.smooth_tail = smooth_tail
\end{lstlisting}

\paragraph{Run CAST with DFTB}\mbox{}\\

If you want to run CAST with DFTB, set the interface parameter in the CAST inputfile to DFTB. To interact with DFTBaby it is necessary to copy the file ``dftbaby\_interface.py'' into the folder where you run CAST. You also have to provide the path to DFTBaby (.../DFTBaby-\#\#\#) as DFTBath. Now you can run a SP or GRAD calculation with a tinkerstructure (LOCOPT and MD should also work). The output of DFTBaby is written into the file ``output\_dftb.txt''. All DFTB options apart from DFTBath are optional. They are explained in the inputfile ``CAST.txt'' or in the documentation for DFTBaby.
