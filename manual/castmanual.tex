...
\subsubsection{MOCK}
This interface calculates energy values of a single atom from a function specified in \texttt{MOCK\_function}.
Its purpose is to investigate if energy barriers of a certain height can be overcome in MD simulations.
The variables are \texttt{x}, \texttt{y} and \texttt{z}.
The functions have the following form (denoted in EBNF):
\begin{lstlisting}
...
ident    ::= /[a-z]+/
\end{lstlisting}
The available operations are addition (\texttt{+}), subtraction (\texttt{-}), multiplication (\texttt{*}) and exponentiation (\texttt{\^}) with the usual precedence rules and left-associativity.
Note that while the parser can handle arbitrary expressions as exponents, due to limitations of the differentiation systems only numeric literals can be exponents!
Mathematical functions are the exponential function (\texttt{exp}) and the square root (\texttt{sqrt}).
