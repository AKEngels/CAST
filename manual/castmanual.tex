...
CAST tries to assign atom types according to the OPLSAA force field to the atoms. To find correct peptide bonds, which are necessary for getting atom types, the atom names of the proteine backbone have to be C, O, CA, N, H and no other atom must have one of these atom types. Not all amino acids and protonation states are implemented yet but you should get a warning if you have a residue that is not implemented. If you want to complement the implementation you can do this in file ``coords\_io\_PDB.cc'', function ``find\_at\_sidechain''. Aside of proteins CAST also assignes atom types to the residues ``Na+'' (sodium ion) and ``WAT'' (water).
...
\textbf{Recommendation!} The atomtype finder for xyz-files is much better tested than the atomtype finder for pdb-files so it might be a good idea first to convert your pdb-file to xyz (e.\,g. by the task \texttt{WRITE\_XYZ}) and then read it as xyz and get the atomtypes from there.
...
This task moves your structure to the origin of the coordinate system. Either the center of mass or the center of geometry are moved to the origin, i.\,e. the point (0/0/0) in the cartesian coordinate system. Which of those two options happens can be determined by the user with the option \texttt{MOVEmode} in the inputfile \texttt{CAST.txt}.
    ...
    \textbf{Warning!} Periodic boundaries can only be used together with the forcefield interfaces or the QM/MM and Three-layer interfaces! If you use a subtractive QM/MM scheme with periodic boundaries the outermost layer has to be a forcefield!
