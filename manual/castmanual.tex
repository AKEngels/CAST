...
\textbf{Attention!} Currently there is some kind of error in the function that calculates $\Delta G$ out of $\Delta E$. However the values for $\Delta E$ are given in the file ``alchemical.txt'' so $\Delta G$ can be calculated from this file without the use of CAST. You can use the script ``FEP\_check.py'' that is found in the directory ``\textbackslash optional\_files\textbackslash scripts''. For every window it prints the $\Delta G$ value of the window and the total $\Delta G$ until the window.
