...
As user input you only need a \texttt{PARAMETER\_LIST} in \texttt{fake\_queue.py} and one in \texttt{umbrella2d\_helper.py}. The first contains the values for the x-coordinate (i.\,e. the first torsion), the second contains the values for the y-coordinate (i.\,e. the second torsion). In our case both are [-180, -175, ..., 0, 5, 10, ..., 180].
...
To start the WHAM calculation we use a slightly modified \texttt{umbrella2d\_analysis} script that has to be started in the folder \texttt{path}. In the folder \texttt{manual/Tutorial/scripts} it is called \texttt{umbrella2d\_analysis\_path.py}.
