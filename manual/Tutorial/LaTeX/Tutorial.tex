...
\begin{titlepage}
	\begin{titlepage}
		\begin{center}
			...
			\Large{Authors: Julian Erdmannsdörfer and Daniel Bellinger\\[1cm]
				Last Updated: \today }
		\end{center}
	\end{titlepage}
\end{titlepage}
...
\paragraph{From scratch}IN a first step we may want to introduce how to generate TINKER files from scratch. The two programs we will need are molden\myCite{MOLDEN} and tinker. If there is no PDB file of our target molecule first of all we need to generate this. A very handy program to do so is molden\myCite{MOLDEN}.
...
\paragraph{Generating the final tinker-file}The most straight forward way to change the MM3 parameters to OPLS-AA ones is to look inside the tinker file and, for each atom, look up the kind of atom molden has assigned it to. So let's take this little example:
...
So we change the atom type to 230 changing the atom's abbreviation is optional. The first atom now looks like:
...
So each value for each keyword can be changed to adjust the settings to each respective task which is desired. So let us take a look at the predefined configuration file. The first section contains general settings starting at line \ref{appCAST:castconf} like the amount of information which is printed during the execution which can be changed in line \ref{appCAST:verbosity} by altering the value next to \texttt{verbosity}. The next value depends on the cores which your PC can spend for the desired calculation. With an \textit{i-X}-Processor there are four cores available and thus the value next to \texttt{cores} in line \ref{appCAST:cores} reads \texttt{4}.\\
The next section is for determining the input and output options beginning at line \ref{appCAST:filestypes} and starting with the name of the molecules file name in our case we got to write next to \texttt{name} the name of the Alanine Glycine peptide, so \texttt{AlaGly.xyt} will do. The next option in line \ref{appCAST:output} decides in which file the final output is written. In this case we use \texttt{AlaGly\_final}. Keep in mind that there is no ending needed depending on the method there is a ending generated. As we use a tinker-file as input format we write in line \ref{appCAST:intype} \texttt{TINKER}. Of course there are other input types, too. %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Welche inputtypes gibt es noch????
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
At line \ref{appCAST:tasks} begins a listing of all possible tasks which we depict in this tutorial and which are possible to undergo. The task we desire is to write in line \ref{appCAST:task} next to \texttt{task}. In a first step we want to start a local optimization for which we choose \texttt{LOCOPT}. This is the only configuration we can change in this section. The next section concentrates on the energy interfaces \ac{eg} OPLS-AA which we desire to use. To do so we need to write \texttt{OPLS-AA} next to \texttt{interface} in line \ref{appCAST:interface}. We do not need a preinterface (for preoptimization) so we let the value in line \ref{appCAST:preinterface} unchanged. %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%What is a preinterface?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The path to the parameter files needs to be adjusted as well and so the desired parameter file needs to be defined in line \ref{appCAST:paramfile}. So we change the value to \texttt{OPLS-AA.prm} and copy the FF file to the directory with the \texttt{CAST.txt} and the \texttt{AlaGly.xyz}. Everything is set up for the first optimization so we open a command prompt and enter: 
\begin{lstlisting}[frame=single,]
	cd "C:\PATH\TO\TUTORIAL"
	"C:\PATH\TO\CAST\CAST.exe"
\end{lstlisting}

CAST starts and prints the local optimized structure in the desired outputfile with a \texttt{\_LOCOPT.arc} extension. Opening this shows the result in appendix \ref{app:finalarc}.


\section{Available energy interfaces}

CAST posses in principle three different types of energy interfaces. Starting with force fields which are directly included in the CAST code. Therefore, the CHARMM, AMBER, OPLS-AA and AMOEBA force fields (FF) are available. In addition to the standard AMOEBA FF. The improved SAPT-FF is included (details will be given in the specific paragraph). The second type are semi-empirics, whereas we use a an interface to the MOPAC program from the Stewart group. The third interface offers the possibility to do DFT calculations on GPUs by using an MPI interface with the Terachem (V1.5) program from Todd Martinez group.
...
\paragraph{AMOEBA and SAPT-FF}
The AMOEBA FF belongs to the class of polarizable FFs. The energy description should not be described in detail. A detailed description can be found in \myCite{Ponder2010}  The SAPT-FF is a specialized kind of the AMOEBA FF, whereas the short-range electrostatics are treated within a partitioning scheme.
...
by a set of multipole moments.\myCite{Stone1996} But it is shown that
...
The SAPT-FF calculation needs several additional input files. The minimum which requested is the Spackman.prm file which contains the $\kappa$ and the atomic basis information. If one would like to use the interpolative variant the precalculated energy and gradient lists are needed. These files are called XY\_EN.in or XY\_GRAD.in. For a successful calculation one needs all the .in files (at the moment 20 files). The Spackman.prm file is given in the appendix (see line \ref{appCAST:spackman}). Within the parameter files the $\kappa$ values can be changed (see line \ref{appCAST:spackmankappa}. The first number indicates the atom type by the atomic charge value.


\end{itemize}
\paragraph{MOPAC}

By using the the \texttt{MOPAC} interface keyword one needs to install the latest Version of MOPAC (MOPAC2012/16) to do calculations. For academic groups MOPAC can be freely downloaded from: \textit{http://openmopac.net/downloads.html}. The MOPAC interface is an file I/O interface and needs some input parameters to generate the appropriate input file. Generally the MOPAC interface can be combined with all existing tasks which need an energy or gradient evaluation. The most important keywords are:
...
\section{Go through tasks}

\paragraph{SP}Now we may compare the total energy of our ``initial guess'' and the local minimum we found. To do so we can look at the \texttt{AlaGly\_final\_LOCOPT.txt} file to obtain the energy of the local minimum which reads $-125.525~kcal/mol$ but if we close the command prompt the energy of the initial structure is gone. So we can compute this by doing a \ac{sp} calculation by just changing the task to \texttt{SP}. Remember the task was in line \ref{appCAST:task} in the input file seen in appendix \ref{app:casttxt} and again running CAST with
...
and we obtain a total energy of $5.204~kcal/mol$. The same way one could calculate the gradients by using \texttt{GRAD}.
