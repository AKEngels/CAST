...
Before we continue we shoud repeat the calculations which are broken. If you are lucky then nothing is broken and you can continue below with detailed plotting. If not go in the corresponding folders and repeat the calculations. As they are quite short the best might be to do this on the headnode so you just type \texttt{.\textbackslash CAST.exe} into the terminal.

After all those calculations have been performed again rename the files \texttt{umbrella.txt} to \texttt{umbrella\_x\_y.txt} and copy them into the analysis folder. Then you have to run WHAM again. For this you can just submit the script \texttt{pbs.sh} which has been written before by typing \texttt{qsuv pbs.sh}.

After it has finished you can replot the free energy and the probability with the script \texttt{umbrella2d\_plot.py}. Now the result should look like shown in figure \ref{fig:umbrella2d_new}. As you see the green spot in the middle now has disappeared and everything is like we would expect it.
...
\begin{figure} [htb]
	...
	\label{fig:umbrella2d_x150}
\end{figure}

The Free Energy and the probability look very nice as we would expect it. 
...
We see that all those pictures are quite nice. However if we look at the numbers in the Free Energy plot they are at around 900 which is quite much and doesn't make any sense. This is because we have added up all 73 plots in the direction which is not plotted which is not correct. What is correct is adding up the probabilities, so those graphs are correct not only in shape but also in numbers. If we wanted to calculate the total free energy in one direction out of our simulations what we would have to do is to take this total probabiliy distribution and calculatate the free energy by the formula

\todo{add formula}

However as we see in our example the sum of all the free energies in one direction can also be a hint if everything has gone as it should, even though it doesn't make any sense physically. 
...
Because we already ran all the MDs we won't repeat them but just take the results of those MDs that we want to take into account now. These are for the first step those that lie on the path. So we create a folder \texttt{path} and copy into it all \texttt{umbrella\_x\_y.txt} files where x and y are in the lowest energy path (\texttt{path.csv}). If we are too lazy to do this by hand we can use a small script like the following:
\begin{lstlisting}[frame=single,language=python,basicstyle=\footnotesize,commentstyle=\color{red},keywordstyle=\color{blue},stringstyle=\color{mygreen},numbers=left]
import shutil
...
To start the WHAM calculation we use a slightly modified \texttt{umbrella2d\_analysis} script that has to be started in the folder \texttt{path}:
\begin{lstlisting}[frame=single,language=python,basicstyle=\footnotesize,commentstyle=\color{red},keywordstyle=\color{blue},stringstyle=\color{mygreen},numbers=left]
...
COMBINATIONS = [[-180, -180], [-180, -175], [-175, -175], [-175, -170], [-170, -170],
...
As input we only have to give the name of the csvfile that contains the surface and the path as a list of points on the surface. The energy of the path that is plotted by this script is shown in figure \ref{fig:path_walk}
...
This looks quite different from figure \ref{fig:path} where the same path is plotted just from the WHAM calculation where all data points are taken into account, not just those from lowest energy path. Furthermore an angle of -180° is identical to one of +180° which has a consequence that starting and endpoint of the ``reaction'' are the same and should have the same energy. In figure \ref{fig:path} this is the case (the difference is $0.26 \frac{kcal}{mol}$) but here the difference is around $50 \frac{kcal}{mol}$ which is quite a lot. However if we just look at the first half of the path the figures are quite similar. We see two peaks which have a height of about $6 \frac{kcal}{mol}$ (be aware that the graph in figure \ref{fig:path_walk} doesn't start which 0!) and then at step 50 the energy starts rising quite strongly. But while in figure \ref{fig:path} the energy reaches a maximum at step 67 (this is surface point -35, 5) in figure \ref{fig:path_walk} the energy continues rising until around step 100 (surface point -35, 170). After that the relative energies seem quite okay. In both paths there are two more maxima, one at around step 110 and one around step 130.
