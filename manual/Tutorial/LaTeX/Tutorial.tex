...
Furthermore this script creates a scatterplots of every simulation (for an example see \ref{fig:scatter_one}) and of all simulations together (\colorbox{red}{TODO: PUT IN PICTURE}). The single scatterplots should ideally look like the one shown and be placed near the corresponding restraints (i.\, e. in our case X = 90, Y = 105). The total scatterplot should cover the whole surface. If parts are missing these parts of the surface are not described well and the free energy for these regions is not reliable.

\begin{figure}[htb]
	\center
	\includegraphics[width=0.5\textwidth]{Img/umbrella_90_105.png}
	\caption{Example for a scatterplot of one distribution (X = 90, Y = 105).}
