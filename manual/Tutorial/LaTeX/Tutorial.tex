...
\begin{figure} [htb]
	...
	\subfigure[Free Energy (Heatmap)]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy2D_new.png}}
	\subfigure[Probability (Heatmap)]{\includegraphics[width=0.49\textwidth]{Img/Probability_heatmap.png}}
...
To check if files are broken and if yes which files you can use the script \texttt{umbrella2d\_check.py}. The input for \texttt{WINDOWS\_X} and \texttt{WINDOWS\_Y} is the same as in \texttt{umbrella2d\_analysis.py}. What is does is to plot probability distributions for the single simulations as we have done it for the one-dimensional umbrella sampling, too (see figure \ref{fig:distribution}). But as we now have two dimensions it is not that easy to plot. That's why it first chooses any value for x and plots the y-values of the corresponding simulations and then it goes through all values for y and plots the x-values. So in the end we have $73 + 73 = 146$ plots which should all look like the one in figure \ref{fig:distributions2d}a because our example is very symmetric and the two reaction coordinates are more or less independent of each other. If an \texttt{umbrella.txt} file is broken there are a lot of zeroes in it so the probability distribution might look like \ref{fig:distributions2d}b. The names of the broken files are also written into a file \texttt{broken\_files.txt}. Normally you should see an empty list there (\texttt{[]}) so no files are broken. 

\textbf{Attention!} Newer versions of this script (found in folder \texttt{optional\_files/scripts/umbrella\_stuff}) also create scatterplots of the single simulations and all simulations together. This is not possible in our case because we have too many simulations and too many points in each simulation to save them all and plot them afterwards. However in case of a real umbrella sampling those scatterplots are often more informative than the distributions we plot here. The total scatterplot should cover the whole surface or at least all regions that are relevant for your interpretation of the results.

If you don't have any broken files you can continue below with \textbf{Detailed Plotting}. Otherwise you should just restart the broken calculations until all plots look okay. 
...
In our case it is not necessary because we use the same simulation data as before and have already looked at them but in general we should check if all of the \texttt{umbrella.txt} files are okay and the distributions overlap each other. This we do by copying the script \texttt{umbrella2d\_check.py} into folder \texttt{path} and run it. It again plots all available distributions and scatterplots and writes the name of files into \texttt{broken\_files.py}. This last file should now contain an empty list (\texttt{[]}) as no files are broken. The pictures of the distributions only those simulations that are taken into account (so many of the images contain only one distribution) but they should still have the same shape and if there are several distributions in one picture they should overlap each other. If a total scatterplot was created (not the case with our script) you should be able to see that the path you want to calculate is nicely covered.
