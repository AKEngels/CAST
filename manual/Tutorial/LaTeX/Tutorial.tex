...
\begin{itemize}
	...
	\item If our reaction coordinate were a distance we would give it as \texttt{USdist}. Because this is not the case we comment out this line.
...
\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/free_180.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/prob_180.png}}
...
\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/Probability.png}}
...
The script we use now is a bit more complicated than the above one because it doesn't send all the calculations at once but uses a timing. You don't have to understand in detail what it's doing so for a better explanation of the script \texttt{fake\_queue.py} we refer to the AKEngels Wiki. So you should start the script in a terminal where it's okay if the script is running for some hours. The script will call two other scripts called \texttt{python.sh} and \texttt{umbrella2d\_helper.py} which must also be copied into the folder. 

As user input you only need a \texttt{PARAMETER\_LIST} in \texttt{fake\_queue.py} and one in \texttt{umbrella2d\_helper.py}. The first contains the restraint values for the x-coordinate (i.\,e. the first torsion), the second contains those for the y-coordinate (i.\,e. the second torsion). In our case both are [-180, -175, ..., 0, 5, 10, ..., 180].
...
As user input we need the path to \texttt{wham2d}. As \texttt{WINDOWS\_X} and \texttt{WINDOWS\_Y} we have to give a list of the restraint values for the x- and y-coordinate respectively (identical to \texttt{PARAMETER\_LIST}) before. \texttt{FORCE\_CONSTANT\_X} and \texttt{FORCE\_CONSTANT\_Y} are the two force constants (in our case they are identical at 0.05). WHAM also needs the minimum and maximum values as well as the number of bins for both directions. We give the same values here as for the one-dimensional umbrella above. Furthermore we have to give the temperature and a tolerance which is also the same as before. The only really new parameter is \texttt{AUTO\_MASK}. We will see later what this is, for now we just deactivate it.

Again we start our script with \texttt{python umbrella2d\_analysis.py} and wait...\\

After the script has finished (this takes some hours) we can look at the output. The interesting stuff again is in the output file \texttt{out.txt}. It contains four columns: In the first and second are the reaction coordinates x and y (i.\,e. the two torsional angles), the third contains the free energy for this combination of reaction coordinates and the fourth the probability. 

The first thing we want to do is to plot those two things in a heatmap. For this we use the script \texttt{umbrella2d\_plot.py}. This script needs a bit of a user input: \texttt{X\_MIN}, \texttt{X\_MAX} and \texttt{X\_STEP} as well the as the same variables for \texttt{Y} refer to the values in the file \texttt{out.txt} so we don't have any other possibility what we give here. \texttt{X\_LABEL\_STEP} and \texttt{Y\_LABEL\_STEP} determine the distance of the labels in the heatmap which we can choose free but always have to be a multiple of \texttt{X\_STEPS} or \texttt{Y\_STEPS}. So we could have given 10 or 90 instead of 30 but not 12 because this is not a multiple of 5. After copying this script into the \texttt{analysis} folder and running it we get two pictures \texttt{FreeEnergy.png} and \texttt{Probability.png} which are shown in figure \ref{fig:umbrella_2d}. Furthermore the free energy is written into a csv file \texttt{freeEnergy.csv} which we will use later and the probability is also written into a csv file which is called \texttt{Probability.csv}.

\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy2D.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/Probability2D.png}}
	\caption{Pictures with Free Energy and Probability for 2D umbrella sampling}
	\label{fig:umbrella_2d}
\end{figure}

The free energy looks again as expected: It is lowest in the corners of the plot where both torsions are at 180° and highest in the middle of the plot where both torsions are 0°. You can also quite well see the other maxima where the torsional angles are -120 or +120°. The probability has quite a bad resolution but you can see the peak probability at the corners and -- with a bit of imagination -- also the smaller peaks for torsional angles of $\pm$ 60°. What is strange is the green spot exactly in the middle of the free energy plot which means that the free energy is lower there than in the places around it. Normally you don't have this spot because there was an error in the program when the tutorial was written. If you can see it probably some of your \texttt{umbrella.txt} files are broken. 

To check if files are broken and if yes which files you can use the script \texttt{umbrella2d\_check.py}. The input for \texttt{WINDOWS\_X} and \texttt{WINDOWS\_Y} is the same as in \texttt{umbrella2d\_analysis.py}. What is does is to plot probability distributions for the single simulations as we have done it for the one-dimensional umbrella sampling, too (see figure \ref{fig:distribution}). But as we now have two dimensions it is not that easy to plot. That's why it first chooses any value for x and plots the y-values of the corresponding simulations and than it goes through all values for y and plots the x-values. So in the end we have $73 + 73 = 146$ plots which should all look like the one in figure \ref{fig:distributions2d}a because our example is very symmetric and the two reaction coordinates are more or less independent of each other. If an \texttt{umbrella.txt} file is broken there are a lot of zeroes in it so the probability distribution might look like \ref{fig:distributions2d}b. The names of the broken files are also written into a file \texttt{broken\_files.txt}. Normally you should see an empty list there (\texttt{[]}) so no files are broken. If this is the case you can continue below with \textbf{Detailed Plotting}.

\begin{figure} [htb]
	\subfigure[good (X = -85)]{\includegraphics[width=0.49\textwidth]{Img/distribution_X-85.png}}
	\subfigure[bad (Y = -180)]{\includegraphics[width=0.49\textwidth]{Img/distribution_Y-180.png}}
	\caption{Examples for distributions in 2D umbrella sampling}
	\label{fig:distributions2d}
\end{figure}

If you have broken calculations you shoud repeat them. Just go into the corresponding folders and repeat the calculations. As they are quite short the best might be to do this on the headnode so you just type \texttt{.\textbackslash CAST.exe} into the terminal.

After all those calculations have been performed again rename the files \texttt{umbrella.txt} to \texttt{umbrella\_x\_y.txt} and copy them into the \texttt{analysis} folder. Then you have to run WHAM again. For this you can just submit the script \texttt{pbs.sh} which has been written before by typing \texttt{qsub -V pbs.sh}.

After it has finished you can replot the free energy and the probability with the script \texttt{umbrella2d\_plot.py}. Now the result should look like shown in figure \ref{fig:umbrella2d_new} (without the red lines). As you see the green spot in the middle now has disappeared and everything is like we would expect it.

\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/freeEnergy2D_new.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/Probability2D_new.png}}
	\caption{Pictures with Free Energy and Probability for 2D umbrella sampling after rerunning broken calculations}
	\label{fig:umbrella2d_new}
\end{figure}

\textbf{Detailed Plotting}\\
In order to have a better view over one dimension you can also cut the 3-dimensional free-energy surface and only plot the other dimension. This can be done by the script \texttt{umbrella2d\_cut.py}.

As user input we have to give the direction which we want to set fixed (in our case it's 'X') and the value where we want to cut (here it's 175°). So you can imagine drawing a vertical line at 175° in figure \ref{fig:umbrella2d_new} and plotting everything which is there. We again plot the free energy on this line (\texttt{FreeEnergy\_X175.png}) and the probability (\texttt{Probability\_X175.png}). These two pictures are shown in figure \ref{fig:umbrella2d_x175}. The free energy and the probability look very nice. 

\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy_X175.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/Probability_X175.png}}
	\caption{Free Energy and Probability for torsional angle X = 175°}
	\label{fig:umbrella2d_x175}
\end{figure}

To demonstrate that we can cut the surface in both directions we now set \texttt{DIRECTION} to 'Y' and \texttt{VALUE} to 120 in order to plot the free energy and probability on a horizontal line at 120°. The results are shown in figure \ref{fig:umbrella2d_y120}.

\begin{figure} [htb]
	\subfigure[Free Energy]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy_Y120.png}}
	\subfigure[Probability]{\includegraphics[width=0.49\textwidth]{Img/Probability_Y120.png}}
	\caption{Free Energy and Probability for torsional angle Y = 120°}
	\label{fig:umbrella2d_y120}
\end{figure}

The free energy looks similar as before which is also expected because the two torsional angles are structural identical. The probability is also quite similar but it looks a bit strange because the values are very low (with around 0.00002 at the highest point). This is because with 120° for Y we chose a probability minimum whereas with 175° for X we were near the maximum.

Instead of cutting the 3-dimensional surface we can also add up one direction and plot the other. This can be done with the script \texttt{umbrella2d\_add.py}. As user input we again have to give a direction, in this case the one that should be plotted (i. e. the other one gets added up). For \texttt{MIN}, \texttt{MAX} and \texttt{STEP} we have to give the minimum and maximum value for that direction in file \texttt{out.txt} as well as the difference between the values. This is the same as given in the scripts \texttt{umbrella2d\_check.py} or \texttt{umbrella2d\_plot.py}. We first plot direction X, then start the script again with direction Y. The results are shown in figure \ref{fig:umbrella_add}.

\begin{figure} [htb]
	\subfigure[Sum of Free Energy in X direction]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy_X.png}}
	\subfigure[Probability in X direction]{\includegraphics[width=0.49\textwidth]{Img/Probability_X.png}}
	\subfigure[Sum of Free Energy in Y direction]{\includegraphics[width=0.49\textwidth]{Img/FreeEnergy_Y.png}}
	\subfigure[Probability in Y direction]{\includegraphics[width=0.49\textwidth]{Img/Probability_Y.png}}
	\caption{Added up Free Energies and Probabilities in both directions}
	\label{fig:umbrella_add}
\end{figure}

We see that all those pictures look quite nice in shape. However if we look at the numbers in the free energy plot they are at around 900 which is quite much and doesn't make any sense. This is because we have added up all 73 plots in the direction which is not plotted and this is not correct. What is correct is adding up the probabilities, so those graphs are correct not only in shape but also in numbers. If we wanted to calculate the total free energy in one direction out of our simulations what we would have to do is to take this total probability distribution and calculatate the free energy by the formula $A(\xi) = -k_B T \cdot \ln P(\xi)$. However as we see in our example the sum of all the free energies in one direction can also be a hint whether everything has gone as it should, even though this plot doesn't make any sense physically. 

\FloatBarrier

Often when doing a two-dimensional umbrella-sampling we are not interested in the whole surface but only in the lowest energy path for a reaction. To get the energies of this path it should not be necessary to perform MDs on every point on the surface but only on those points that are on or near the path. This is what we want to try now. But before we can try this we need to know what our lowest energy path might be. To find it we open the file \texttt{freeEnergy.csv} which was created before. It contains the free energy surface as an excel sheet. Our ``reaction'' should be a complete rotation of both main torsions of pentane from -180° at the beginning to +180° at the end. So what we are looking for is the path with the lowest values in our table starting at the upper left corner and going to the lower right corner. We could do this by hand in the excel sheet but we can also use the script \texttt{walkThroughSurface.py}

As input this script needs the corner where we want to start (in our case it's \texttt{up\_left}) and the name of the csv file with the surface. We copy this script in our \texttt{analysis} folder and start it. It creates three output files: \texttt{pes.tex}, \texttt{path.csv} and \texttt{path.png}. First we look at \texttt{pes.tex}. This is a LaTeX file with the same table as the inputfile but now the lowest energy path is marked in blue. So we have to compile this texfile and the result should look like in figure \ref{fig:surface_with_path}.

\begin{figure}[htb]
	\center
	\includegraphics[width=\textwidth]{Img/pes_with_path.png}
	\caption{Free Energy Surface with lowest energy path marked in blue}
	\label{fig:surface_with_path}
\end{figure}

So in principal the blue cells are everything we need to know. With a little less noise data this is also shown in the excel sheet \texttt{path.csv}. Its first column contains a step number, the next two columns the x- and y-coordinate of the surface and the last value is the free energy at this point of the surface. The picture \texttt{path.png} is just a plot the first and last column of this file. It might look as shown in figure \ref{fig:path}.

\begin{figure}[htb]
	\center
	\includegraphics[width=0.7\textwidth]{Img/path.png}
	\caption{Free Energy on lowest energy path for pentane}
	\label{fig:path}
\end{figure}

\vspace{0.5cm}
Now that we know where the lowest energy path is and how the energy should look like on it we want to try to get the energy of the path without sampling the whole surface but only the structures on the path. This means we don't have 5329 surface points any more, but just 145 (see in \texttt{path.csv}).

Because we already ran all the MDs we won't repeat them but just take the results of those MDs that we want to take into account now. These are those that lie on the path. So we create a folder \texttt{path} and copy into it all \texttt{umbrella\_x\_y.txt} files where x and y are in the lowest energy path (\texttt{path.csv}). If we are too lazy to do this by hand we can use a small script \texttt{copy\_files.py}.

In our case it is not necessary because we use the same simulation data as before and have already looked at them but in general we should check if all of the \texttt{umbrella.txt} files are okay and the distributions overlap each other. This we do by copying the script \texttt{umbrella2d\_check.py} into folder \texttt{path} and run it. It again plots all available distributions and writes the name of files into \texttt{broken\_files.py}. This last file should now contain an empty list (\texttt{[]}) as no files are broken. The pictures of the distributions only those simulations that are taken into account (so many of the images contain only one distribution) but they should still have the same shape and if there are several distributions in one picture they should overlap each other.

To start the WHAM calculation we use a slightly modified \texttt{umbrella2d\_analysis.py} script that has to be started in the folder \texttt{path}. In the folder \texttt{manual/Tutorial/scripts} it is called \texttt{umbrella2d\_analysis\_path.py}.

The user input remains the same, except that \texttt{AUTO\_MASK} is now set to \texttt{True}. This is necessary because we now have bins in our surface for which there is no data. This feature tells WHAM to exclude those bins from the calculation. Furthermore we need to give a list of \texttt{COMBINATIONS} for which we have done an MD simulation (i.\,e. those from the path). Later in the script the function \texttt{create\_inputfile()} is a bit different from before and thus has to called a bit differently in the last paragraph of the code.

This calculation is much shorter than the one before (it needs just around 15 minutes) so by excluding points on the surface which are not interesting to us we don't just save time for the MD simulations but also time for the analysis with WHAM. If we look at the outputfile \texttt{out.txt} we see that for a lot of points the free energy is 9999999 and the probability is 0. These are probably the points which are not included in the calculation. Let's see what happens when we plot the free energy with the script \texttt{umbrella2d\_plot.py}. We don't have to change anything in there just copy it into the \texttt{path} folder and run it. The result is shown in figure \ref{fig:freeE_path}.

\begin{figure}[htb]
	\center
	\includegraphics[width=0.7\textwidth]{Img/FreeEnergy_path.png}
	\caption{Free Energy from WHAM calculation where only surface points from lowest energy path are taken into account}
	\label{fig:freeE_path}
\end{figure}

We see that most of the surface is yellow which means the energy is 9999999 and there's no data for that point. We also see that the violet points are where the lowest energy path was before (compare figure \ref{fig:surface_with_path}) which is good because we hope to get reasonable data for this path.

The next thing we want to do is to walk on the path and plot the energy. For this we can't use the \texttt{walkThroughSurface.py} script because it searches the lowest energy path on a surface. As our surface contains a lot of bullshit entries the path it would find would probably also be bullshit (you can try if you want). Instead we have to tell where the path is that we want to walk on. For this we use the script \texttt{walkOnPath.py}.

As input we only have to give the name of the csv file that contains the surface and the path as a list of points on the surface. The energy of the path plotted by this script is shown in figure \ref{fig:path_walk}.

\begin{figure}[htb]
	\center
	\includegraphics[width=0.7\textwidth]{Img/path_walk.png}
	\caption{Free Energy on lowest energy path for pentane if only surface points from lowest path are taken into account for WHAM calculation}
	\label{fig:path_walk}
\end{figure}

This plot looks quite similar as figure \ref{fig:path} where we have plotted the same path but it was calculated from a lot more MD simulations. So we have shown that if we are only interested in the energies on a specific path that we already know it is enough to run MD simulations on this path and we don't have to plot the whole surface. This saves a lot of time and computer power.
...
#USdist                 <index 1> <index 2> <force> <r0> <flat bottom 0/1> <width>
#USdist                 1 2 155.0 1.09 1 0.2
