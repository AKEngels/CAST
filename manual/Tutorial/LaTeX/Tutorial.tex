...
while True:
    ...
    if jobs_to_be_sent < 0:
        ...
        counter = start_next_jobs(counter, jobs_to_be_sent)
\end{lstlisting}
{\raggedleft\texttt{fake\_queue.py}}

\begin{lstlisting}[frame=single,language=bash,basicstyle=\footnotesize,numbers=left,escapechar=|]
#PBS -l nodes=1:ppn=4                                                           
#PBS -l mem=4020MB
...
def run_calc(param):
    ...
    # user variables for calculation
    PLACEHOLDER = "RESTRAINT_2"       # placeholder in CAST.txt file
    ...
    shutil.copy("/apps/CAST/CAST/optional_files/build/CAST_linux_x64_release", 
    ...
    for s1 in STEPS_1:
        for s2 in STEPS_2:
            ...
            if os.path.isfile("f_{}/f_{}/umbrella.txt".format(s1,s2)):
                # copy and rename file
                shutil.copy("f_{}/f_{}/umbrella.txt".format(s1,s2), "analysis/umbrella_{}_{}.txt".format(s1,s2))
                # write inputfile for WHAM
                with open("analysis/in.txt","a") as inp:         
                    inp.write("umbrella_{}_{}.txt   {}  {}   {}  {}\n".format(s1,s2,float(s1), float(s2),
                                                                              FORCE_CONSTANT_1, FORCE_CONSTANT_2))
            else:
                print "No file 'umbrella.txt' in folder", s
                sucess = False
    return success
...
Because now the WHAM calculation lasts much longer it is submitted to the queue by the function \texttt{submit\_pbs()}. If you don't want this but instead want to run \texttt{wham2d} on the headnode (not recommended on UNICORN!) just replace the last line by:
\begin{lstlisting}[frame=single,language=python,basicstyle=\footnotesize,commentstyle=\color{red},keywordstyle=\color{blue},stringstyle=\color{mygreen},numbers=left,firstnumber=86]
    os.popen(order)
\end{lstlisting}

Most of the user input is self-explanatory as it is quite the same as for 1D-WHAM. The only thing that is new is the parameter \texttt{AUTO\_MASK}. We will see later what this is, for now we just deactivate it.
