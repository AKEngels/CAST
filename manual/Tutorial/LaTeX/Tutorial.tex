...
So first we need our input structure, butan.arc, as tinkerfile:
\begin{lstlisting}[frame=single,]
14      (oplsaa_mod2.prm)
 1  C     -5.942760    2.779360    0.148310     80     2     3     4     5
 2  C     -5.217520    1.481290   -0.169120     81     1     6     7     8
 3  H     -7.014540    2.669170   -0.044280     85     1
 4  H     -5.578860    3.597360   -0.481310     85     1
 5  H     -5.811080    3.062510    1.197470     85     1
 6  C     -3.751490    1.455970    0.267600     81     2     9    10    11
 7  H     -5.280610    1.294710   -1.248420     85     2
 8  H     -5.739010    0.651730    0.322560     85     2
 9  C     -2.952440    2.679850   -0.151080     80     6    12    13    14
10  H     -3.280410    0.560620   -0.154690     85     6
11  H     -3.696970    1.353180    1.358550     85     6
12  H     -3.068160    2.883970   -1.220280     85     9
13  H     -1.888730    2.523830    0.054180     85     9
14  H     -3.267820    3.565990    0.408600     85     9
\end{lstlisting}

Then we go into the inputfile CAST.txt.
\begin{itemize}
	\item The name of our inputfile is of course butan.arc, the inputtype is TINKER.
	\item The task is UMBRELLA.
	\item Our energy interface is OPLSAA.
	\item The paramfile is called oplsaa\_mod2.prm. You can find this file in the Github Repository in optional\_files/forcefields\_utility.
	\item Then we scroll down a lot to the section MD OPTIONS.
	\item We want to sample the molecule for 500\,ps, so we choose 500000 MDsteps. Velocity scaling may be switched on by giving option 1 for MDveloscale.
	\item We use the Velocity-Verlet integrator, so the value for integrator is 0.
	\item Because we want to keep the temperature at 300\,K by the Nosé-Hoover thermostat we set MDtemp\_control and MD\_thermostat to 1. Furthermore we set MDheat to ``0    300'' which means that the thermostat should try to set the temperature to 300\,K immediately (at step 0).
	\item For MDtimestep we use the default timestep of 0.001\,ps, we switch off MDrestart\_if\_broken (hopefully our molecule won't break) and switch on MDtrack.
	\item 1000 snapshots should be enough so we set MDsnap to 1000, MDsnapbuffer might be 100 and the snapshots mustn't be optimized so we set MDsnap\_opt to 0.
	\item The rest of the MD options is not necessary.
	\item Then we scroll to UMBRELLA SAMPLING.
	\item Before every sampling there is a short MD simulation to eqilibrate the system. We want to equilibrate the system for 10000 steps so we set USequil to 10000. 
	\item The main torsion of our butane molecule is defined by the atoms 1, 2, 6 and 9. It should be constrained to an angle of 150° (this is just an example we could also use any other value between -180° and +180°) with a spring constant of $0.05 \frac{kcal}{mol \cdot deg^2}$. So the option for UStorsion looks like that: ``UStorsion              1 2 6 9  0.05 150.0''.
	\item If our reaction coordinate was a distance we would give it as USbond. Because this is not the case we comment out this line.
	\item For the sampling we want to use the conformations of every step. So we set USsnap to 1.
\end{itemize}

Now we run CAST. After the calculation has finished we have three output files: A tracefile, a snapshot file and a file that is called umbrella.txt.
...
The last file is that called umbrella.txt. It contains the value of the restrained torsion for every step of the sampling simulation. As we saw in the snapshot file all the values are quite near to 150°. This file will later be used by WHAM to calculate the PMF.
