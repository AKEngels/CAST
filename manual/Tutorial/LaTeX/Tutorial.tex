...
MAX_JOBS = 25      # this is the maximum number of jobs I want to send to queue        |\label{maxJobs}|
DELAY = 300        # time in seconds for which the program waits before looking again  |\label{delay}|
...
def run_calc(param):
    ...
    PLACEHOLDER_1 = "RESTRAINT_1"       # placeholders in CAST.txt file 
...
This script does in principle the same as the \texttt{umbrella.py} script we used before. For every single calculation we need it creates a folder, copies the necessary files into it, modifies the input and submits the calculation. Only two things are different:
\begin{enumerate}
	\item Because now we have two parameters we change and a big number of calculations there is a hierachy in the folders. So to get into the folder of the calculation where e.\,g. the first torsional angle is 50° and the second is 30° you first have to go into a folder called \texttt{f\_50} and in this folder you wilf find a subfolder called \texttt{f\_30} where the calculation is stored.
	\item Also because of the big number of calculations there is a timing in submitting them so that not all of them get on the cluster at the same time. You can control the timing by the variables \texttt{MAX\_JOBS} and \texttt{DELAY} in lines \ref{maxJobs} and \ref{delay}. With the first you tell the script the maximum number of jobs you want to send to the cluster at once (in our case 25 so after you have sent 25 calculations the script won't send any more) and with the second number you determine the interval where the script looks whether it can send new calculations. In our case it's 300 seconds.\\
	So when you start the script it will look how many jobs you have already sent (it also counts jobs that you sent before manually as it just reads \texttt{qstat}). Then it sends as much of them until you have 25 jobs in the queue. Then it waits 5 minutes until it looks again. If jobs have finished (which will be the case as each calculations only needs a few seconds) it again sends as many new ones until the total number of your calculations in the queue is 25. Then it again waits 5 minutes. This will continue until all of the calculations are sent. In the meantime you can just work on any other stuff, also submit more calculations, even if then you have more than 25 jobs in the queue. The script will just notice that and stop submitting calculations until you're again below 25 jobs in the queue.
\end{enumerate}
