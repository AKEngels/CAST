...
TEST(sorted_indices_from_cs_string, withProperInput)
{
  ...
  std::vector<std::size_t> sorted = sorted_indices_from_cs_string(input);
...
TEST(sorted_indices_from_cs_string, FailsWithFloatingPointInput)
{
  ...
  ASSERT_THROW(sorted_indices_from_cs_string(input), std::runtime_error);
}
...
TEST(sorted_indices_from_cs_string, throwsWhenNegativeNumbersArePresent)
{
  ...
  ASSERT_THROW(sorted_indices_from_cs_string(input2), std::runtime_error);
}
